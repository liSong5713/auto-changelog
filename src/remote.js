...
function getRemote(remoteURL, options = {}) {
  ...
  if (IS_GITLAB) {
    const url = IS_GITLAB_SUBGROUP
      ? `${protocol}//${hostname}/${remote.repo}/${remote.branch.replace(
          /\.git$/,
          ''
        )}`
  ...
  if (IS_ELONG_GITLAB) {
    const url = `http://code.corp.elong.com/${remote.repo}`;
    return {
      getCommitLink: id => `${url}/commit/${id}`,
      getIssueLink: id => `${url}/issues/${id}`,
...
function getOverrides({ commitUrl, issueUrl, mergeUrl, compareUrl }) {
  ...
  if (compareUrl)
    overrides.getCompareLink = (from, to) =>
      compareUrl.replace('{from}', from).replace('{to}', to);
  return overrides;
}
